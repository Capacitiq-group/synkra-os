...
//
// Worker auth, domain normalization, event logging, and the suppression
// helper now live in ./acquisition_lib.js — required from inside each
...
routerAdd("POST", "/api/acquisition/companies/ingest", (e) => {
  ...
  const lib = require(`${__hooks}/acquisition_lib.js`);

  lib.requireAcquisitionWorkerAuth(e);
  const data = e.requestInfo().body;
  const domain = lib.normalizeDomain(data && data.domain);
  if (!domain || !data.company_name) {
    throw new shared.ApiError(400, "domain and company_name are required.");
  }

  let company = shared.tryFindFirst(e.app, "prospect_companies", "domain = {:domain}", { domain });
  const isNew = !company;
  if (!company) {
  ...
  if (isNew) {
    lib.logAcquisitionEvent(e.app, "company_discovered", { companyId: company.id, detail: { source: data.source } });
  }
...
routerAdd("POST", "/api/acquisition/companies/{id}/research", (e) => {
  ...
  lib.requireAcquisitionWorkerAuth(e);
  const data = e.requestInfo().body;
  const company = shared.findOrNotFound(e.app, "prospect_companies", e.request.pathValue("id"), "Company");
  ...
  lib.logAcquisitionEvent(e.app, data.research_status === "qualified" ? "qualified" : "research_completed", {
...
routerAdd("POST", "/api/acquisition/contacts/ingest", (e) => {
  ...
  if (!data.company_id || !data.name) {
    throw new shared.ApiError(400, "company_id and name are required.");
  }
  shared.findOrNotFound(e.app, "prospect_companies", data.company_id, "Company");

  let contact = data.business_email
    ? shared.tryFindFirst(e.app, "prospect_contacts", "business_email = {:email}", { email: data.business_email })
    : null;
  const isNew = !contact;
  if (!contact) {
  ...
  if (isNew) {
    lib.logAcquisitionEvent(e.app, "contact_found", { companyId: data.company_id, contactId: contact.id });
  }
  if (data.verification_status === "verified") {
    lib.logAcquisitionEvent(e.app, "contact_verified", { companyId: data.company_id, contactId: contact.id });
  }
...
routerAdd("POST", "/api/acquisition/campaigns/create", (e) => {
  ...
  if (!data.contact_id || !data.company_id) {
    throw new shared.ApiError(400, "contact_id and company_id are required.");
  }
  const contact = shared.findOrNotFound(e.app, "prospect_contacts", data.contact_id, "Contact");
  const company = shared.findOrNotFound(e.app, "prospect_companies", data.company_id, "Company");

  if (company.get("already_contacted")) {
    throw new shared.ApiError(409, "This company has already been contacted — refusing to create a duplicate initial-outreach campaign.");
  }
  if (contact.get("business_email")) {
    const emailNormalized = String(contact.get("business_email")).toLowerCase().trim();
    const suppressed = shared.tryFindFirst(e.app, "suppression_list", "email_normalized = {:email}", { email: emailNormalized });
    if (suppressed) {
      throw new shared.ApiError(403, `This contact is on the suppression list (reason: ${suppressed.get("reason")}). No campaign was created.`);
    }
  }
...
routerAdd("POST", "/api/acquisition/campaigns/{id}/event", (e) => {
  ...
  if (!eventType) throw new shared.ApiError(400, "event_type is required.");

  const campaign = shared.findOrNotFound(e.app, "acquisition_campaigns", e.request.pathValue("id"), "Campaign");
  const contact = shared.findOrNotFound(e.app, "prospect_contacts", campaign.get("contact"), "Contact");

  if (campaign.get("suppressed") && ["email_sent", "followup_sent"].includes(eventType)) {
    throw new shared.ApiError(403, "This campaign is suppressed. No send may proceed regardless of what the caller requests.");
  }
  ...
  switch (eventType) {
    case "email_sent":
      ...
      {
        const company = shared.findOrNotFound(e.app, "prospect_companies", campaign.get("company"), "Company");
    ...
    case "reply_classified":
      ...
      if (data.classification === "unsubscribe") {
        ...
        lib.addToSuppressionList(e.app, contact, "unsubscribe");
    ...
    case "deal_lost":
      ...
      break;
    case "email_bounced":
      lib.addToSuppressionList(e.app, contact, "hard_bounce");
      campaign.set("suppressed", true);
  ...
  lib.logAcquisitionEvent(e.app, eventType, {
...
routerAdd("GET", "/api/acquisition/companies/ready-for-outreach", (e) => {
  const lib = require(`${__hooks}/acquisition_lib.js`);
...
routerAdd("GET", "/api/acquisition/suppression/check", (e) => {
  ...
  if (!email) throw new shared.ApiError(400, "email query param is required.");
...
routerAdd("POST", "/api/acquisition/suppression/add", (e) => {
  ...
  const employee = shared.requirePermission(e, "acquisition.manage");
  const data = e.requestInfo().body;
  const email = data && data.email;
  if (!email) throw new shared.ApiError(400, "email is required.");
  const emailNormalized = email.toLowerCase().trim();

  const existing = shared.tryFindFirst(e.app, "suppression_list", "email_normalized = {:email}", { email: emailNormalized });
  ...
  lib.logAcquisitionEvent(e.app, "suppression_added", { detail: { email: emailNormalized, reason: (data && data.reason) || "manual" } });
...
routerAdd("GET", "/api/acquisition/dashboard", (e) => {
  ...
  shared.requirePermission(e, "acquisition.view");
